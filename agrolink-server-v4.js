...
function formatPost(post) {
    ...
    // 📷 Profil resmi thumbnail URL'si ekle (feed'de düşük çözünürlük)
    if (p.userProfilePic && p.userProfilePic.includes('/uploads/profiles/')) {
        const picFilename = p.userProfilePic.split('/').pop();
        p.userProfilePicThumb = `/uploads/profiles/thumbs/${picFilename}`;
    } else {
    ...
    try {
        ...
        if (req.file) {
            ...
            try {
                ...
                // 📷 Thumbnail: Feed'de düşük çözünürlük için 96x96
                const thumbDir = path.join(profilesDir, 'thumbs');
                if (!require('fs').existsSync(thumbDir)) require('fs').mkdirSync(thumbDir, { recursive: true });
                await sharp(req.file.path).resize(96, 96, { fit: 'cover' }).webp({ quality: 70 }).toFile(path.join(thumbDir, filename));
            } catch (e) {
        ...
        await Promise.all([
            (async () => {
                ...
                await sharp(file.path, { sequentialRead: true })
                    ...
                    .toFile(outputPath);
                // 📷 Thumbnail: Feed'de 96x96 küçük versiyon
                const thumbDir = path.join(profilesDir, 'thumbs');
                if (!require('fs').existsSync(thumbDir)) require('fs').mkdirSync(thumbDir, { recursive: true });
                await sharp(file.path, { sequentialRead: true })
...
app.get('/api/users/search', authenticateToken, async (req, res) => {
    try {
        ...
        // 📷 Thumb URL ekle
        users.forEach(u => {
            if (u.profilePic && u.profilePic.includes('/uploads/profiles/')) {
                const picFilename = u.profilePic.split('/').pop();
                u.profilePicThumb = `/uploads/profiles/thumbs/${picFilename}`;
            } else {
                u.profilePicThumb = u.profilePic;
            }
        });
