...
app.post('/api/messages', authenticateToken, messageLimiter, checkRestriction('message'), async (req, res) => {
    try {
        ...
        // ── SOCKET EMIT — gerçek zamanlı iletim ──────────────────────
        const newMessage = {
            id              : msgId,
            senderId        : req.user.id,
        ...
        // Alıcıya gönder
        if (typeof io !== 'undefined' && typeof onlineUsers !== 'undefined') {
            const recipientSockets = onlineUsers.get(recipientId);
            if (recipientSockets) {
                for (const sid of recipientSockets) {
                    io.to(sid).emit('newMessage', { message: newMessage });
                }
            }
            // Gönderenin diğer açık sekmelerine/cihazlarına da gönder
        ...
        res.status(201).json({ message: 'Mesaj gönderildi', id: msgId, messageId: msgId });
    } catch (error) {
...
app.get('/api/messages/:userId', authenticateToken, async (req, res) => {
    try {
        ...
        await dbRun(
            `UPDATE messages SET "isRead" = TRUE, read = TRUE, "readAt" = NOW()
...
app.post('/api/messages/image', authenticateToken, upload.single('image'), async (req, res) => {
    try {
        ...
        // ── SOCKET EMIT — resimli mesaj gerçek zamanlı iletim ──
        if (typeof io !== 'undefined' && typeof onlineUsers !== 'undefined') {
            const imgMsg = {
                id: msgId, senderId: req.user.id, senderUsername: sender.username,
                recipientId, recipientUsername: recipient.username,
                content: imageUrl, mediaUrl: absoluteUrl(imageUrl), mediaType: 'image',
                read: false, isRead: false, createdAt: new Date().toISOString(),
            };
            const recipientSockets = onlineUsers.get(recipientId);
            if (recipientSockets) { for (const sid of recipientSockets) io.to(sid).emit('newMessage', { message: imgMsg }); }
            const senderSockets = onlineUsers.get(req.user.id);
            if (senderSockets) { for (const sid of senderSockets) io.to(sid).emit('newMessage', { message: imgMsg }); }
        }
    ...
    try {
        ...
        if (typeof io !== 'undefined' && typeof onlineUsers !== 'undefined') {
            const newMsg = {
                id: msgId, senderId: req.user.id, senderUsername: sender.username,
                recipientId, recipientUsername: recipient.username,
                content: voiceUrl, mediaUrl: absoluteUrl(voiceUrl), mediaType: 'voice',
                read: false, isRead: false, createdAt: new Date().toISOString(),
            };
            const recipientSockets = onlineUsers.get(recipientId);
            if (recipientSockets) { for (const sid of recipientSockets) io.to(sid).emit('newMessage', { message: newMsg }); }
            const senderSockets = onlineUsers.get(req.user.id);
            if (senderSockets) { for (const sid of senderSockets) io.to(sid).emit('newMessage', { message: newMsg }); }
        }
