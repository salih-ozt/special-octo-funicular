...
app.get('/api/feed', authenticateToken, async (req, res) => {
    try {
        ...
        // cursor: son görülen postun createdAt timestamp'i (ISO string)
        // ilk sayfa için cursor yok, sonraki sayfalar için ?cursor=2024-01-01T00:00:00Z
        ...
        //  Etkileşim          → log scale, sınırlı (viral içerik gelir ama çok dominan olmaz)
        ...
        const feedPosts = await dbAll(
            `SELECT
                ...
                -- Çeşitlilik için rastgele gürültü (daha geniş aralık)
                + (RANDOM() * 8)
             ...
             WHERE p."isActive" = TRUE
               ...
               -- Cursor: bu timestamp'ten önceki postlar (aynı post tekrar gelmez)
               AND p."createdAt" <= $4::timestamptz
               -- Keşfet gönderileri max 14 gün eski
               AND (
                   p."userId" = $1
             ...
             LIMIT $2 OFFSET $3`,
            [req.user.id, lim, 0, cursorStr]
        );

        // Sonraki sayfa için cursor = en düşük skorlu postun tarihi
        const nextCursor = feedPosts.length === lim
            ? feedPosts[feedPosts.length - 1]?.createdAt || null
            : null;

        console.log(`[Feed] userId=${req.user.id} cursor=${cursorStr} dönen=${feedPosts.length}`);

        res.json({
            posts     : feedPosts.map(formatPost),
            nextCursor,
            hasMore   : feedPosts.length === lim,
            algorithm : 'balanced-v2',
        });
    } catch (error) {
