/*
 * =============================================================================
 * AGRO SOSYAL SERVER - PostgreSQL v7.0 (MOBİL + WEB)
 * =============================================================================
 * 
 * 🚀 v7 YENİLİKLER:
...
async function sendWelcomeEmail(userEmail, userName) {
    return sendEmail(userEmail, "🌾 Agro Sosyal'e Hoş Geldiniz!", getWelcomeEmailTemplate(userName));
}
...
function getInactiveUserEmailTemplate(userName, userId) {
    ...
    return `<!DOCTYPE html>
<html lang="tr">
<head><meta charset="UTF-8"><title>Seni Özledik - Agro Sosyal</title>
...
function getHighEngagementEmailTemplate(userName, userId) {
    ...
    return `<!DOCTYPE html>
<html lang="tr">
<head><meta charset="UTF-8"><title>Teşekkürler - Agro Sosyal</title>
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;background:#060d0a;color:#e8f5e9}.wrapper{max-width:600px;margin:0 auto;padding:24px 16px}.hero{background:linear-gradient(135deg,#0a1f10,#0d2b16);border-radius:28px;padding:48px 40px;text-align:center;border:1px solid rgba(0,230,118,0.15)}.brand{font-size:26px;font-weight:800;color:#00e676}.cta{display:inline-block;margin-top:24px;padding:14px 36px;background:linear-gradient(135deg,#00e676,#1de9b6);color:#020810;font-weight:800;border-radius:50px;text-decoration:none}.footer{text-align:center;padding:24px 20px;color:rgba(255,255,255,0.3);font-size:12px}.footer a{color:rgba(0,230,118,0.7);text-decoration:none}</style>
</head>
<body><div class="wrapper">
  <div class="hero">
...
function getTwoFactorEmailTemplate(userName, code, purpose = 'login') {
    ...
    return `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Doğrulama Kodu - Agro Sosyal</title>
<style>
...
function getEmailVerificationTemplate(userName, code) {
    userName = escapeHtml(userName);
    return `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>E-Posta Doğrulama - Agro Sosyal</title>
<style>
body{font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;line-height:1.8;color:#333;margin:0;padding:0;background-color:#f4f4f4}
.container{max-width:600px;margin:20px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.1)}
...
const pool = new Pool({
    ...
    database: process.env.DB_NAME     || 'agro sosyal',
    ...
    application_name           : 'agro sosyal_server',
});
...
//  - SQLITE_PATH env varıyla sqlite dosya konumunu belirtebilirsin (varsayılan: ./agro sosyal.db).
//
//  Kullanım:
//    SQLITE_MIGRATE=true SQLITE_PATH=./agro sosyal.db node agro sosyal-server-pg-FIXED.js
//

const MIGRATION_FLAG = '.migration_done';
const SQLITE_PATH    = process.env.SQLITE_PATH || './agro sosyal.db';
...
async function runSQLiteMigration() {
    ...
    console.log('\n🔄 ============================================');
    console.log('   AGRO SOSYAL — SQLite → PostgreSQL Migrasyonu ');
...
async function initializeDatabase() {
    ...
    const columnMigrations = [
        ...
        // 🎵 Postlara müzik (sound) referansı
        `ALTER TABLE posts ADD COLUMN IF NOT EXISTS "soundId" TEXT`,
    ];
        ...
        const message = {
            ...
            android: {
                ...
                notification: {
                    title,
                    body,
                    channelId: 'agro sosyal_notifications',
                    sound: 'default',
...
// 🎵 Müzik kütüphanesi — admin (sen) buraya .mp3 dosyalarını manuel atar
...
function formatPost(post, soundsMap) {
    if (!post) return post;
    const p = { ...post };
    ...
    // Frontend PostCard bileşeni şu alanları bekler:
    //   post.caption  → post içerik metni
    //   post.media    → [{ url, type }] dizisi
    //   post.author   → { name, avatar }
    //   post.sound    → { id, title, artist, url } | null

    // caption: content alanının alias'ı
    p.caption = p.content || '';

    // media: mediaUrls ile eşle — frontend PostCard bu alanı kullanır
    if (Array.isArray(p.mediaUrls) && p.mediaUrls.length > 0) {
        p.media = p.mediaUrls.map(item => ({
            url : item.url,
            type: item.type || (item.mediaType) || 'image',
        }));
    } else if (p.mediaUrl) {
        p.media = [{ url: p.mediaUrl, type: p.mediaType || 'image' }];
    } else {
        p.media = [];
    }

    // author: kullanıcı bilgilerini iç içe nesneye taşı
    p.author = {
        name  : p.name   || p.username || p.authorUsername || 'Kullanıcı',
        avatar: p.profilePic || null,
    };

    // sound: soundId varsa detayları doldur
    if (p.soundId) {
        // soundsMap opsiyonel parametre — verilmişse tam detay, yoksa temel bilgi
        const soundMeta = soundsMap && soundsMap[p.soundId];
        p.sound = {
            id    : p.soundId,
            title : soundMeta ? soundMeta.title  : p.soundId,
            artist: soundMeta ? soundMeta.artist : '',
            url   : soundMeta ? soundMeta.url    : absoluteUrl(`/uploads/m%C3%BCzik/${encodeURIComponent(p.soundId + '.mp3')}`),
        };
    } else {
        p.sound = null;
    }
...
function mobileKeyMiddleware(req, res, next) {
    ...
    // Sayfa istekleri (/, /agro sosyal vb.) Origin header göndermez;
    ...
    const accessToken = jwt.sign(
        {
            ...
            iss      : 'agro sosyal',         // Issuer
            aud      : 'agro sosyal-client',  // Audience
        },
        JWT_SECRET,
        { expiresIn: '1d', algorithm: 'HS256' }
    );
    const refreshToken = jwt.sign(
        { id: user.id, type: 'refresh', iss: 'agro sosyal' },
        JWT_REFRESH_SECRET,
...
function setAuthCookies(res, req, tokens) {
    ...
    res.cookie('refresh_token', tokens.refreshToken, {
        ...
        maxAge   : 90 * 24 * 60 * 60 * 1000, // 90 gün — mobil uzun süre açılmasa da oturum korunur
    });
}
...
app.post('/api/auth/verify-otp', otpLimiter, validateAuthInput, async (req, res) => {
    ...
    if (tempToken && code) {
        ...
        try {
            ...
            const tokenHash = crypto.createHash('sha256').update(tokens.refreshToken).digest('hex');
            await dbRun(
                `INSERT INTO refresh_tokens (id, "userId", "tokenHash", ip, "userAgent", "createdAt", "expiresAt") VALUES ($1,$2,$3,$4,$5,NOW(),NOW() + INTERVAL '90 days')`,
                [uuidv4(), user.id, tokenHash, req.ip, req.headers['user-agent'] || '']
            );
...
// ════════════════════════════════════════════════════════════════════
// 🎵 POPÜLER SESLER (MÜZİK KÜTÜPHANESİ)
// ────────────────────────────────────────────────────────────────────
// uploads/müzik/ klasöründeki .mp3 dosyaları — admin manuel ekler.
// Dosya adı = ses ID'si (örn. "yaz_havasi.mp3" → id: "yaz_havasi")
// İstersen aynı isimde .json metadata koyabilirsin:
//   uploads/müzik/yaz_havasi.json  → { "title": "Yaz Havası", "artist": "Anonim", "trending": true }
// ════════════════════════════════════════════════════════════════════
async function listSoundLibrary() {
    try {
        ...
        for (const file of mp3s) {
            ...
            const metaFile= path.join(musicDir, `${id}.json`);
            let meta = { title: id, artist: '', trending: false };
            try {
                const raw = await fs.readFile(metaFile, 'utf-8');
                meta = { ...meta, ...JSON.parse(raw) };
            } catch (_) { /* metadata yok */ }
            let duration = null, size = null;
            try {
                const st = await fs.stat(path.join(musicDir, file));
                size = st.size;
            } catch (_) {}
            items.push({
                id,
                ...
                url      : absoluteUrl(`/uploads/müzik/${encodeURIComponent(file)}`),
                duration,
                size,
            });
        }
        // Trending önce, sonra alfabetik
        items.sort((a, b) => (b.trending - a.trending) || a.title.localeCompare(b.title, 'tr'));
        return items;
    } catch (e) {
        console.warn('🎵 Ses kütüphanesi okunamadı:', e.message);
        return [];
    }
}

// GET /api/sounds → tüm sesler (popüler + normal)
app.get('/api/sounds', authenticateToken, async (req, res) => {
    try {
        const { q = '', trending } = req.query;
        let items = await listSoundLibrary();
        if (trending === 'true' || trending === '1') items = items.filter(s => s.trending);
        if (q && q.trim()) {
            const needle = q.toLowerCase();
            items = items.filter(s =>
                s.title.toLowerCase().includes(needle) ||
                (s.artist && s.artist.toLowerCase().includes(needle))
            );
        }
        // Frontend uyumluluğu: hem { sounds } hem { data } hem de düz dizi olarak döner
        // SoundPicker bileşeni bu formatların hepsini extractSounds() ile işler
        res.json({ sounds: items, data: items, items, results: items });
    } catch (error) {
        console.error('Sounds list hatası:', error);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});

// GET /api/sounds/:id → tek bir ses (post oynatıcısı için)
app.get('/api/sounds/:id', authenticateToken, async (req, res) => {
    try {
        const items = await listSoundLibrary();
        const found = items.find(s => s.id === req.params.id);
        if (!found) return res.status(404).json({ error: 'Ses bulunamadı' });
        res.json({ sound: found });
    } catch (error) {
        console.error('Sound get hatası:', error);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});
...
app.post('/api/posts', authenticateToken, checkRestriction('post'), upload.array('media', 10), async (req, res) => {
    try {
        // Frontend caption gönderir, server content okur — ikisini de kabul et
        const { content: _content, caption: _caption, isPoll, pollQuestion, pollOptions, latitude, longitude, locationName, allowComments = 'true', uploadedUrls: uploadedUrlsRaw, soundId = null } = req.body;
        const content = (_content || _caption || '');
        ...
        // 🎵 soundId doğrulama — gönderilen id müzik klasöründe var mı?
        let validSoundId = null;
        if (soundId && typeof soundId === 'string' && soundId.trim()) {
            const cleanId = soundId.trim();
            try {
                const candidate = path.join(musicDir, `${cleanId}.mp3`);
                if (fssync.existsSync(candidate)) validSoundId = cleanId;
            } catch (_) { validSoundId = null; }
        }
        ...
        const insertResult = await pool.query(
            `INSERT INTO posts (id, "userId", username, content, media, "mediaType", "mediaUrls", "mediaWidth", "mediaHeight",
             ...
             latitude, longitude, "locationName", "allowComments", "soundId", "isActive", "createdAt", "updatedAt")
            ...
            [postId, req.user.id, user.username, content || '', media, mediaType, mediaUrlsJson,
             ...
             locationName || null, allowComments !== 'false', validSoundId]
        );
        ...
        // Sound detaylarını doldur (formatPost'a soundsMap gönder)
        let soundsMap = {};
        try {
            const soundList = await listSoundLibrary();
            soundsMap = Object.fromEntries(soundList.map(s => [s.id, s]));
        } catch (_) {}

        res.status(201).json({ success: true, message: 'Gönderi paylaşıldı', post: formatPost(postRow, soundsMap) });
    } catch (error) {
...
app.get('/api/feed', authenticateToken, async (req, res) => {
    try {
        ...
        // Sound detaylarını doldur (soundId olan post varsa)
        let soundsMap = {};
        const hasSoundPosts = posts.some(p => p.soundId);
        if (hasSoundPosts) {
            try {
                const soundList = await listSoundLibrary();
                soundsMap = Object.fromEntries(soundList.map(s => [s.id, s]));
        ...
        const responseData = {
            posts       : posts.map(p => formatPost(p, soundsMap)),
            page        : parseInt(page),
...
app.get('/api/posts/:id', authenticateToken, async (req, res, next) => {
    ...
    try {
        ...
        if (post.soundId) {
            try {
                const soundListSingle = await listSoundLibrary();
                soundsMapSingle = Object.fromEntries(soundListSingle.map(s => [s.id, s]));
            } catch (_) {}
        }
        res.json({ post: formatPost(post, soundsMapSingle) });
    } catch (error) {
...
app.put('/api/posts/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const { content, allowComments, locationName, soundId } = req.body;
        const updates = [];
        const params  = [];
        let idx = 1;
        ...
        if (soundId !== undefined) {
            // null/'' → kaldır, dolu → klasörde dosya var mı kontrol
            let v = null;
            if (soundId && typeof soundId === 'string' && soundId.trim()) {
                const cleanId = soundId.trim();
                try {
                    if (fssync.existsSync(path.join(musicDir, `${cleanId}.mp3`))) v = cleanId;
                } catch(_) {}
            }
            updates.push(`"soundId" = $${idx++}`); params.push(v);
        }
...
const agro sosyalDir = path.join(__dirname, 'public', 'agro sosyal');
if (fssync.existsSync(agro sosyalDir)) {
    app.use('/agro sosyal', express.static(agro sosyalDir, { maxAge: '1d', dotfiles: 'deny' }));
}
app.get('/agro sosyal', (req, res) => {
    const htmlPath = path.join(__dirname, 'public', 'agro sosyal', 'index.html');
...
// ── React Router için SPA catch-all: /agro sosyal/* ──────────────────────────
// Vite build çıktısı public/agro sosyal/ klasörüne kopyalanmalıdır.
// Tüm alt rotalar (/agro sosyal/feed, /agro sosyal/profile vb.) index.html'e yönlendirilir.
app.get('/agro sosyal/*', (req, res) => {
    const htmlPath = path.join(__dirname, 'public', 'agro sosyal', 'index.html');
    if (fssync.existsSync(htmlPath)) {
        ...
        res.status(404).json({ error: 'Agro Sosyal uygulaması bulunamadı. public/agro sosyal/index.html ekleyin.' });
    }
});
...
const CACHE_NAME = 'agro sosyal-v1';
const APP_SHELL = ['/'];
...
<div class="container">
  <div class="card">
    ...
    <a class="cta" href="${DOMAIN}">📱 Agro Sosyal'te Aç</a>
  </div>
...
<div class="wrap">
  ...
  <div class="cta-area"><a class="cta" href="${DOMAIN}">📱 Agro Sosyal'te Aç</a></div>
  <div class="foot">🌾 <span>Agro Sosyal</span> — Tarım Topluluğu</div>
</div>
</body></html>`);
    } catch (e) {
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>${user.name} (@${user.username}) — Agro Sosyal</title>
<meta property="og:title" content="${user.name} - Agro Sosyal">
...
<div class="container">
  ...
  <div class="profile-card">
    ...
    <a href="${base}/u/${user.username}" class="cta">🌾 Agro Sosyal'te Görüntüle</a>
  </div>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>${post.userName} Agro Sosyal Gönderisi</title>
<meta property="og:title" content="${post.userName} - Agro Sosyal">
...
<div class="container">
  ...
  <div class="cta-section">
    <a href="${base}" class="cta">🌾 Agro Sosyal'te Görüntüle →</a>
  </div>
...
async function buildOfficialVerificationPDF({ frontImagePath, backImagePath, userName, username, email, refCode, createdAt }) {
  ...
  /Contents 8 0 R >>`);
    ...
    ops.push(`(AGRO SOSYAL - Kimlik Dogrulama Belgesi) Tj`);
    ops.push(`/F2 10 Tf`);
    ops.push(`0 -18 Td`);
...
app.post('/api/verification/request', authenticateToken, upload.fields([
    ...
    try {
        ...
        const attachments = [];
        if (pdfBuf) attachments.push({ filename: `agro sosyal-kimlik-${refCode}.pdf`, content: pdfBuf });
    ...
    try {
        ...
        try {
            const gcRes = await fetch(
                ...
                { headers: { 'User-Agent': 'AgroSosyal/1.0 (agro sosyal.app)' } }
            );
            if (gcRes.ok) {
...
// ════════════════════════════════════════════════════════════════════════════
// 🤝 PARTNERLİK & İŞ BAŞVURUSU ENDPOINTLERİ (sadece worker'larda)
// ════════════════════════════════════════════════════════════════════════════
...
app.post('/api/partnership/apply', async (req, res) => {
    try {
        ...
        <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:32px;background:#0a1628;border-radius:16px;color:#e2e8f0">
            ...
            <p>Agro Sosyal Tarım Teknolojileri A.Ş. bünyesinde <strong style="color:#22c55e">${workField}</strong> alanında çalışma başvurunuz başarıyla alındı.</p>
            <p>Ekibimiz başvurunuzu inceleyecek ve en kısa sürede size geri dönüş yapacaktır.</p>
            <p style="color:#94a3b8;font-size:13px;margin-top:24px">Agro Sosyal Tarım Teknolojileri A.Ş. — Fatsa/Ordu</p>
        </div>`;

        await sendEmail(email, '✅ Agro Sosyal — Başvurunuz Alındı', userHtml).catch(() => {});
...
app.post('/api/partnership/review/:id', authenticateToken, requireNotBanned, async (req, res) => {
    try {
        ...
        <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:32px;background:#0a1628;border-radius:16px;color:#e2e8f0">
            ...
            ${isApproved
                ? `<p>Agro Sosyal Tarım Teknolojileri A.Ş. bünyesinde <strong style="color:#22c55e">${app_.workField}</strong> alanındaki başvurunuz <strong>onaylandı!</strong></p>
        ...
        await sendEmail(app_.email, isApproved ? '🎉 Agro Sosyal — Başvurunuz Onaylandı' : '❌ Agro Sosyal — Başvuru Sonucu', resultHtml).catch(() => {});
...
(async () => {
    try {
            ...
            server.listen(PORT, '0.0.0.0', async () => {
                console.log(`
╔══════════════════════════════════════════════════╗
║  🌾 AGRO SOSYAL SERVER - PostgreSQL v7.1             ║
