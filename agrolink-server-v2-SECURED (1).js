...
const JWT_SECRET = process.env.JWT_SECRET || (() => {
    if (process.env.NODE_ENV === 'production') {
        console.error('🚨 KRİTİK GÜVENLİK HATASI: Production ortamında JWT_SECRET environment variable ZORUNLUDUR!');
        console.error('   export JWT_SECRET="$(openssl rand -hex 64)"');
        process.exit(1);
    }
    console.warn('⚠️  JWT_SECRET tanımlı değil! Geçici dev key kullanılıyor - Production\'da KULLANMAYIN!');
    return 'dev-only-' + require('crypto').randomBytes(32).toString('hex');
})();
const JWT_REFRESH_SECRET = process.env.JWT_REFRESH_SECRET || (() => {
    if (process.env.NODE_ENV === 'production') {
        console.error('🚨 KRİTİK GÜVENLİK HATASI: Production ortamında JWT_REFRESH_SECRET environment variable ZORUNLUDUR!');
        process.exit(1);
    }
...
app.use(helmet({
    ...
    // 🔒 same-site: Başka sitelerden medya çekilmesini kısıtla
...
app.use(express.json({ limit: '10mb' }));   // ✅ Güvenlik: JSON body max 10MB (API istekleri)
    ...
    try {
        ...
        if (user.twoFactorEnabled === 0) {
            ...
            const token = jwt.sign({ 
                ...
                username: user.username,
                role: user.role
            }, JWT_SECRET, { expiresIn: '15m' }); // 🔒 Access token: 15 dakika; refresh token ile yenile
