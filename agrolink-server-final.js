...
function formatPost(post) {
    ...
    // 📷 Profil resmi thumbnail URL'si (feed'de 96x96 küçük versiyon)
    if (p.userProfilePic && p.userProfilePic.includes('/uploads/profiles/')) {
        p.userProfilePicThumb = `/uploads/profiles/thumbs/${p.userProfilePic.split('/').pop()}`;
    } else {
        p.userProfilePicThumb = p.userProfilePic || null;
    }
    ...
    try {
        ...
        if (req.file) {
            ...
            try {
                ...
                // 📷 Thumbnail 96x96 (feed için)
                const thumbDir1 = path.join(profilesDir, 'thumbs');
                if (!fssync.existsSync(thumbDir1)) fssync.mkdirSync(thumbDir1, { recursive: true });
                await sharp(req.file.path).resize(96, 96, { fit: 'cover' }).webp({ quality: 70 }).toFile(path.join(thumbDir1, filename)).catch(()=>{});
            } catch (e) {
        ...
        await Promise.all([
            (async () => {
                ...
                await sharp(file.path, { sequentialRead: true })
                    ...
                    .toFile(outputPath);
                // 📷 Thumbnail 96x96 (feed için)
                const thumbDirUp = path.join(profilesDir, 'thumbs');
                if (!fssync.existsSync(thumbDirUp)) fssync.mkdirSync(thumbDirUp, { recursive: true });
                await sharp(file.path, { sequentialRead: true })
                    .resize(96, 96, { fit: 'cover', kernel: 'lanczos2' })
                    .webp({ quality: 70, effort: 2 })
                    .toFile(path.join(thumbDirUp, filename)).catch(()=>{});
                await fs.unlink(file.path).catch(() => {});
...
// ═══════════════════════════════════════════════════════
// 🌾 /agro — Ana uygulama kısa yolu
// ═══════════════════════════════════════════════════════
app.get('/agro', (req, res) => {
    const htmlPath = path.join(__dirname, 'public', 'index.html');
    if (fssync.existsSync(htmlPath)) return res.sendFile(htmlPath);
    res.redirect('/');
});
app.get('/agro/', (req, res) => {
    const htmlPath = path.join(__dirname, 'public', 'index.html');
...
// ═══════════════════════════════════════════════════════
// 📄 /hakkimizda — Google marka sayfası (SEO kritik)
// Kendi HTML'ini public/hakkimizda.html olarak kaydet → otomatik açılır
// ═══════════════════════════════════════════════════════
...
<title>Hakkımızda | Agro Sosyal</title>
<meta name="description" content="Agro Sosyal, çiftçiler ve tarım sektörü için geliştirilmiş yerli sosyal medya platformu.">
<meta name="robots" content="index, follow">
<link rel="canonical" href="${DOMAIN}/hakkimizda">
<script type="application/ld+json">{"@context":"https://schema.org","@type":"Organization","name":"Agro Sosyal","alternateName":["agrososyal","AgroSosyal"],"url":"${DOMAIN}","logo":{"@type":"ImageObject","url":"${DOMAIN}/agro.png"},"description":"Çiftçiler ve tarım sektörü için geliştirilmiş yerli sosyal medya platformu.","foundingDate":"2024","areaServed":{"@type":"Country","name":"Türkiye"},"inLanguage":"tr-TR","sameAs":["${DOMAIN}","${DOMAIN}/hakkimizda","${DOMAIN}/kullanim","https://www.instagram.com/agrososyal_official"]}<\/script>
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,sans-serif;background:#f8fffe;color:#1a2e1a;line-height:1.7}header{background:linear-gradient(135deg,#00b894,#00a381);padding:16px 24px;display:flex;align-items:center;gap:12px;position:sticky;top:0;z-index:100;box-shadow:0 2px 12px rgba(0,184,148,.2)}header img{width:40px;height:40px;border-radius:10px}header h1{color:#fff;font-size:20px;font-weight:900}header a{color:rgba(255,255,255,.85);text-decoration:none;margin-left:auto;font-size:14px;font-weight:600;border:1px solid rgba(255,255,255,.4);padding:6px 14px;border-radius:20px}.hero{background:linear-gradient(160deg,#00b894 0%,#006d54 100%);color:#fff;padding:64px 24px 80px;text-align:center;position:relative}.hero::after{content:'';position:absolute;bottom:-2px;left:0;right:0;height:50px;background:#f8fffe;clip-path:ellipse(55% 100% at 50% 100%)}.hero h2{font-size:clamp(28px,6vw,48px);font-weight:900;margin-bottom:16px}.hero p{font-size:18px;opacity:.9;max-width:600px;margin:0 auto}.container{max-width:860px;margin:0 auto;padding:48px 24px}section{background:#fff;border-radius:20px;padding:36px;margin-bottom:28px;box-shadow:0 2px 16px rgba(0,0,0,.06);border:1px solid #e8f5f0}section h3{font-size:22px;font-weight:800;color:#00b894;margin-bottom:16px}section p,section li{color:#445544;font-size:16px;margin-bottom:10px}ul{padding-left:20px}.ig-btn{display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888);color:#fff;padding:14px 24px;border-radius:14px;text-decoration:none;font-weight:700;font-size:16px;margin-top:8px}.cta{background:linear-gradient(135deg,#00b894,#00a381);border-radius:20px;padding:40px;text-align:center;color:#fff}.cta h3{color:#fff;font-size:24px;margin-bottom:12px}.cta a{display:inline-block;background:#fff;color:#00b894;font-weight:800;padding:14px 32px;border-radius:50px;text-decoration:none;font-size:16px}footer{text-align:center;padding:32px 24px;color:#889988;font-size:13px;border-top:1px solid #e0ede8}footer a{color:#00b894;text-decoration:none}</style>
</head>
<body>
<header><img src="/agro.png" alt="Agro Sosyal" onerror="this.style.display='none'"><h1>Agro Sosyal</h1><a href="/agro">Uygulamaya Git</a></header>
<div class="hero"><h2>🌾 Hakkımızda</h2><p>Türkiye'nin çiftçi ve tarım sektörü için geliştirilmiş yerli sosyal medya platformu</p></div>
<div class="container">
<section><h3>🌱 Biz Kimiz?</h3><p><strong>Agro Sosyal</strong>, çiftçiler, üreticiler, ziraat mühendisleri ve tarım sektörüyle ilgilenen kullanıcıları bir araya getiren <strong>yerli bir dijital sosyal medya platformudur</strong>. 2024 yılında kuruldu.</p></section>
<section><h3>📸 Instagram'da Bizi Takip Edin</h3><p>Tarımsal içerikler, platform haberleri ve topluluk paylaşımları için Instagram sayfamızı takip edin.</p><a href="https://www.instagram.com/agrososyal_official" target="_blank" rel="noopener noreferrer" class="ig-btn"><svg width="22" height="22" viewBox="0 0 24 24" fill="white"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>@agrososyal_official</a></section>
<section><h3>🎯 Misyonumuz</h3><p>Türkiye'deki milyonlarca çiftçiyi ve tarım emekçisini dijital platformda buluşturarak bilgi paylaşımını ve topluluk dayanışmasını güçlendirmek.</p><ul><li>Çiftçiler arası bilgi ve deneyim paylaşımını kolaylaştırmak</li><li>Tarımsal ürün ve hizmetler için güvenli pazar oluşturmak</li><li>Ziraat mühendisleri ile çiftçileri doğrudan buluşturmak</li></ul></section>
<section><h3>📍 İletişim & Linkler</h3><p><strong>Platform:</strong> <a href="${DOMAIN}/agro" style="color:#00b894">${DOMAIN}/agro</a></p><p><strong>Instagram:</strong> <a href="https://www.instagram.com/agrososyal_official" target="_blank" style="color:#e1306c">@agrososyal_official</a></p><p><strong>Gizlilik & Kullanım:</strong> <a href="/kullanim" style="color:#00b894">${DOMAIN}/kullanim</a></p><p><strong>Kuruluş Yılı:</strong> 2024 | <strong>Hizmet Alanı:</strong> Türkiye</p></section>
<div class="cta"><h3>Hemen Katılın!</h3><p>Türkiye'nin çiftçi sosyal medyasında yerinizi alın. Ücretsiz kayıt olun.</p><a href="/agro">Uygulamaya Git →</a></div>
</div>
<footer><p>&copy; 2024 Agro Sosyal. | <a href="/agro">Ana Sayfa</a> | <a href="/hakkimizda">Hakkımızda</a> | <a href="/kullanim">Kullanım & Gizlilik</a></p></footer>
</body></html>`);
});

// ═══════════════════════════════════════════════════════
// 📄 /kullanim — Gizlilik Politikası & Kullanım Koşulları
// ✅ public/kullanim.html oluşturursan ONU açar (senin HTML'in)
// ═══════════════════════════════════════════════════════
...
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Kullanım & Gizlilik | Agro Sosyal</title>
<meta name="description" content="Agro Sosyal kullanım koşulları ve gizlilik politikası.">
<meta name="robots" content="index, follow">
<link rel="canonical" href="${DOMAIN}/kullanim">
<script type="application/ld+json">{"@context":"https://schema.org","@type":"WebPage","name":"Kullanım Koşulları ve Gizlilik Politikası","url":"${DOMAIN}/kullanim","description":"Agro Sosyal platformu kullanım koşulları ve gizlilik politikası.","inLanguage":"tr-TR","isPartOf":{"@type":"WebSite","name":"Agro Sosyal","url":"${DOMAIN}"}}<\/script>
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,sans-serif;background:#f8fffe;color:#1a2e1a;display:flex;flex-direction:column;min-height:100vh;align-items:center;justify-content:center;text-align:center;padding:40px 24px}header{background:linear-gradient(135deg,#00b894,#00a381);padding:16px 24px;display:flex;align-items:center;gap:12px;position:fixed;top:0;left:0;right:0;z-index:100}header img{width:36px;height:36px;border-radius:8px}header h1{color:#fff;font-size:18px;font-weight:900}header a{color:rgba(255,255,255,.85);text-decoration:none;margin-left:auto;font-size:13px;font-weight:600;border:1px solid rgba(255,255,255,.4);padding:5px 12px;border-radius:16px}.card{background:#fff;border-radius:24px;padding:48px 40px;max-width:500px;box-shadow:0 4px 24px rgba(0,0,0,.08);border:1px solid #e8f5f0;margin-top:80px}.icon{font-size:64px;margin-bottom:20px}h2{color:#00b894;font-size:22px;margin-bottom:12px}p{color:#667766;font-size:15px;margin-bottom:20px;line-height:1.6}.badge{display:inline-block;background:#fff8e1;color:#f59e0b;border:1px solid #fcd34d;border-radius:20px;padding:6px 16px;font-size:13px;font-weight:700;margin-bottom:20px}.hint{background:#f0faf7;border:1px solid #c3e8de;border-radius:14px;padding:16px;font-size:13px;color:#445544;margin-bottom:24px;text-align:left}code{background:#e8f5f0;padding:2px 8px;border-radius:6px;font-family:monospace;font-size:12px}.btn{display:inline-block;background:#00b894;color:#fff;font-weight:700;padding:12px 28px;border-radius:50px;text-decoration:none}footer{text-align:center;padding:24px;color:#889988;font-size:13px;position:fixed;bottom:0;left:0;right:0}footer a{color:#00b894;text-decoration:none}</style>
</head>
<body>
<header><img src="/agro.png" alt="Agro Sosyal" onerror="this.style.display='none'"><h1>Agro Sosyal</h1><a href="/agro">Uygulamaya Git</a></header>
<div class="card">
...
<h2>Kullanım Koşulları &amp; Gizlilik Politikası</h2>
<p>Bu sayfa kısa süre içinde yayına girecek. Kendi HTML içeriğini hazırlayıp sunucuya kaydetmeni bekliyoruz.</p>
<div class="hint">
    <strong>📁 Nasıl yayına girer?</strong><br><br>
    Sunucuda şu dosyayı oluştur:<br>
    <code>public/kullanim.html</code><br><br>
    Dosyayı kaydettiğinde bu sayfa otomatik olarak senin içeriğini gösterir.
</div>
<a href="/agro" class="btn">Ana Sayfaya Dön</a>
</div>
<footer><a href="/hakkimizda">Hakkımızda</a> | <a href="/kullanim">Kullanım & Gizlilik</a></footer>
</body></html>`);
});
...
app.get('/api/users/search', authenticateToken, async (req, res) => {
    try {
        ...
        // 📷 Thumb URL ekle
        users.forEach(u => {
            u.profilePicThumb = (u.profilePic && u.profilePic.includes('/uploads/profiles/'))
                ? `/uploads/profiles/thumbs/${u.profilePic.split('/').pop()}`
                : u.profilePic;
        });
