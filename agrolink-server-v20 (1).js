...
app.post('/api/verification/request', authenticateToken, upload.fields([
    ...
    try {
        ...
        await dbRun(
            ...
            // Tablo yoksa (ilk migrasyon henüz çalışmadıysa) sessizce devam et
            console.warn('verification_requests tablosu bulunamadı, migrasyon bekleniyor');
        });
...
app.get('/api/verification/action/:token/:action', async (req, res) => {
    ...
    try {
        const vReq = await dbGet(
            `SELECT vr.*, u.email, u.username, u."isVerified"
        ...
        if (approved) {
            await dbRun(`UPDATE users SET "isVerified"=TRUE, "updatedAt"=NOW() WHERE id=$1`, [vReq.userId]);
        }
