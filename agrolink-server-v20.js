...
app.post('/api/upload', authenticateToken, upload.single('media'), async (req, res) => {
    try {
        ...
        if (isVideo) {
            ...
            const filename = `post_${Date.now()}_${Math.round(Math.random() * 1E9)}${ext}`;
        ...
        console.error('❌ /api/upload hatası:', err.message);
...
// ─── PDF Oluşturucu (pdfkit gerektirmez, raw PDF binary) ───────────────────
async function buildVerificationPDF(frontImagePath, backImagePath, userName) {
    // JPEG görsellerini oku ve metadata al
    const frontBuf = await fs.readFile(frontImagePath);
    const backBuf  = await fs.readFile(backImagePath);
    const frontMeta = await require('sharp')(frontBuf).metadata();
    const backMeta  = await require('sharp')(backBuf).metadata();

    // Sayfayı A4 (595x842 pt) olarak düzenle, her görseli yatay yarıya yerleştir
    const pageW = 595, pageH = 842, margin = 40, gap = 20;
    const imgW = pageW - 2 * margin;
    const imgH = Math.floor((pageH - 2 * margin - gap - 60) / 2); // 60pt = başlık

    // Görselleri istenen boyuta sığdır
    const scaleImg = async (buf, meta) => {
        const ratio = Math.min(imgW / meta.width, imgH / meta.height);
        return require('sharp')(buf)
            .resize(Math.round(meta.width * ratio), Math.round(meta.height * ratio), { fit: 'inside' })
            .jpeg({ quality: 85 })
            .toBuffer();
    };

    const fBuf = await scaleImg(frontBuf, frontMeta);
    const bBuf = await scaleImg(backBuf,  backMeta);

    const fMeta = await require('sharp')(fBuf).metadata();
    const bMeta = await require('sharp')(bBuf).metadata();
    ...
    // Sayfa
    const pageId = addObj(`<< /Type /Page /Parent 2 0 R
  /MediaBox [0 0 ${pageW} ${pageH}]
  /Resources << /XObject << /FImg 4 0 R /BImg 5 0 R >>
                /Font << /F1 6 0 R >> >>
  /Contents 7 0 R >>`);

    // Görseller
    const fImgId = addObj(`<< /Type /XObject /Subtype /Image
  /Width ${fMeta.width} /Height ${fMeta.height}
  /ColorSpace /DeviceRGB /BitsPerComponent 8
  /Filter /DCTDecode /Length ${fBuf.length} >>`);
    objs[fImgId - 1] = { header: objs[fImgId - 1], stream: fBuf };

    const bImgId = addObj(`<< /Type /XObject /Subtype /Image
  /Width ${bMeta.width} /Height ${bMeta.height}
  /ColorSpace /DeviceRGB /BitsPerComponent 8
  /Filter /DCTDecode /Length ${bBuf.length} >>`);
    objs[bImgId - 1] = { header: objs[bImgId - 1], stream: bBuf };

    // Font
    const fontId = addObj(`<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>`);

    // İçerik akışı
    const fY = pageH - margin - 50 - fMeta.height;
    const bY = fY - gap - bMeta.height;
    const contentStr = [
        'BT',
        '/F1 14 Tf',
        `${margin} ${pageH - margin - 20} Td`,
        `(Agrolink - Kimlik Doğrulama Belgesi) Tj`,
        '/F1 10 Tf',
        `0 -20 Td`,
        `(Kullanici: ${(userName || '').replace(/[()\\]/g, '')}) Tj`,
        `0 -14 Td`,
        `(Tarih: ${new Date().toLocaleDateString('tr-TR')}) Tj`,
        'ET',
        // Ön yüz
        `${margin} ${fY} ${fMeta.width} ${fMeta.height} re S`,
        `q ${fMeta.width} 0 0 ${fMeta.height} ${margin} ${fY} cm /FImg Do Q`,
        // Arka yüz
        `${margin} ${bY} ${bMeta.width} ${bMeta.height} re S`,
        `q ${bMeta.width} 0 0 ${bMeta.height} ${margin} ${bY} cm /BImg Do Q`,
    ].join('\n');
    const contentBuf = Buffer.from(contentStr);
    const contentId = addObj(`<< /Length ${contentBuf.length} >>`);
    objs[contentId - 1] = { header: objs[contentId - 1], stream: contentBuf };
...
// POST /api/verification/request (alias) — PDF üret + e-posta gönder
app.post('/api/verification/request', authenticateToken, upload.fields([
    ...
    try {
        ...
        if (frontFile && backFile) {
            try {
                const pdfBuf = await buildVerificationPDF(frontFile.path, backFile.path, userName);
                pdfAttachment = { filename: 'kimlik-dogrulama.pdf', content: pdfBuf };
            } catch (pdfErr) {
                console.error('PDF oluşturma hatası:', pdfErr.message);
                // PDF oluşturulamazsa görselleri ekle
                pdfAttachment = null;
            }
        }

        // Doğrulama talebi e-postası gönder
        const targetEmail = 'noreply.agrolink@gmail.com';
        const html = `
            <h2>🌱 Agrolink - Kimlik Doğrulama Talebi</h2>
            <p><strong>Kullanıcı Adı:</strong> ${user?.username || req.user.id}</p>
            <p><strong>Ad Soyad:</strong> ${userName || 'Belirtilmedi'}</p>
            <p><strong>E-posta:</strong> ${user?.email || 'Bilinmiyor'}</p>
            <p><strong>Talep Tarihi:</strong> ${new Date().toLocaleString('tr-TR')}</p>
            <hr>
            <p>Kimlik belgeleri ${pdfAttachment ? 'ekte PDF olarak' : 'görseller olarak'} gönderilmiştir.</p>
        `;
        ...
        if (pdfAttachment) {
            ...
            attachments.push({ filename: 'kimlik-on.jpg', path: frontFile.path });
        }
        if (!pdfAttachment && backFile) {
            attachments.push({ filename: 'kimlik-arka.jpg', path: backFile.path });
        }
        ...
        if (transporter) {
            try {
                await transporter.sendMail({
                    from: `Agrolink <${process.env.SMTP_USER || process.env.EMAIL_USER}>`,
                    to: targetEmail,
                    subject: `Kimlik Doğrulama Talebi - ${userName || user?.username || req.user.id}`,
                    html,
                    attachments
                });
                console.log('📧 Kimlik doğrulama e-postası gönderildi:', targetEmail);
            } catch (mailErr) {
                console.error('Kimlik doğrulama e-posta hatası:', mailErr.message);
            }
        }

        // DB güncelle - pending olarak işaretle (admin onayı bekleniyor)
        await dbRun('UPDATE users SET "updatedAt"=NOW() WHERE id=$1', [req.user.id]);

        // Geçici dosyaları temizle
        const cleanupFiles = [frontFile?.path, backFile?.path].filter(Boolean);
        for (const fp of cleanupFiles) {
            await fs.unlink(fp).catch(() => {});
        }

        res.json({ message: 'Doğrulama talebiniz alındı. PDF olarak e-posta gönderildi.', pending: true });
    } catch (e) {
...
// POST /api/verification/remove — Rozeti kaldır + bildirim e-postası gönder
app.post('/api/verification/remove', authenticateToken, async (req, res) => {
    try {
        ...
        // Bildirim e-postası
        const html = `
            <h2>🌱 Agrolink - Doğrulanmış Rozet Kaldırıldı</h2>
            <p>Kullanıcı <strong>${user.username}</strong> (${user.email}) doğrulanmış rozetini kaldırdı.</p>
            <p><strong>Tarih:</strong> ${new Date().toLocaleString('tr-TR')}</p>
            <p>Hesap artık doğrulanmamış olarak işaretlenmiştir.</p>
        `;
        ...
        if (transporter) {
            await transporter.sendMail({
                ...
                html
            }).catch(err => console.error('Rozet kaldırma e-posta hatası:', err.message));
        }
